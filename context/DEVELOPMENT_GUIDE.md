...
1. **Тесты перед кодом.** Каждая новая фича — сначала тест, потом реализация. Текущий стандарт: 386 тестов, 0 ошибок.
...
#### Checklist перед GitHub launch:
- [ ] Зарезервировать `github.com/synoema`
- [ ] `cargo test` = 264 passed, 0 failed
- [ ] README.md актуален
- [ ] LICENSE = MIT
- [ ] .gitignore включает target/
- [ ] `crates.io` — зарезервировать `synoema`
- [ ] Домен `synoema.dev` (через Regway — нейтральная юрисдикция)

#### Статьи (14 файлов готовы):
- [ ] Опубликовать #1 на Хабре
- [ ] Опубликовать #2 через неделю
- [ ] Опубликовать #3 через 2 недели
- [ ] Launch-пост (#6) на HN через 4 недели
- [ ] #4, #5 после запуска
- [ ] #7 (vision) через 2 месяца
